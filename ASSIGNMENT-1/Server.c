...
*/
		if(strcmp(data,"SEND") == 0){
			...
			for(int i = 0 ; i < clientCount; i ++){			
				if ( strcmp(data, (Client[i].username) ) ==0)
				{
					id = i;
				}
			}
			...
			{	
				send(Client[id].sockID,"No guy with that name exists",1024,0);
			}
		}
		...
		if(strcmp(data,"SENDGROUP") == 0){			
			...
			for(int i = 0 ; i < groupCount; i ++){			
				if ( strcmp(data, (Group[i].username_g) ) ==0)
				{
					id = i;
				}
			}
			...
			{
				send(Client[index].sockID,"No Group with that name exists",1024,0);
			}
		...
		if(strcmp(data,"CREATEGROUP") == 0){
			...
			//send(Client[index].sockID,Group[groupCount].username_g,1024,0);
			//printf("here=%s",Group[groupCount].username_g);			
